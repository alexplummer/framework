...
gulp.task('watch', () =>{
	...
	plugins.watch(paths.dev+'/**/*.pug', () => {gulp.start(gulpsync.sync([
		'watch:message','clean:html','html',['bower-inject','clean:prod'],'watch:reload','build:prod'
	]));});
	// JS
	plugins.watch(paths.dev+'/**/*.js', () => {gulp.start(gulpsync.sync([
		'watch:message','clean-reports','js',['copy:scripts','inject-deps'],['lint-reports','clean:prod'],'build:prod'
	]));});
	// IMAGES
